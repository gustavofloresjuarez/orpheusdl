...
from utils.utils import hash_string

module_information = ModuleInformation(
    service_name = 'Qobuz',
    module_supported_modes = ModuleModes.download | ModuleModes.credits,
    global_settings = {'app_id': '', 'app_secret': '', 'quality_format': '{sample_rate}kHz {bit_depth}bit', 'auth_with_tokens': True},
    session_settings = {'username_or_UserID': '', 'password_or_Token': ''},
...
class ModuleInterface:
    def __init__(self, module_controller: ModuleController):
        ...
        self.session = Qobuz(settings['app_id'], settings['app_secret'], settings['auth_with_tokens'], module_controller.module_error) # TODO: get rid of this module_error thing
        self.module_controller = module_controller
        self.session_storage_location = os.path.join('config', 'loginstorage.bin')
        self.temporary_session = read_temporary_setting(self.session_storage_location, 'qobuz')
        self.check_and_login_qobuz(settings)
    ...
    def check_and_login_qobuz(self, settings):
        hashes = {k: hash_string(str(v)) for k, v in settings.items()}
        if not self.temporary_session.get('hashes') or \
            any(k not in hashes or hashes[k] != v for k,v in self.temporary_session['hashes'].items() if k in settings):
            print('Logging into Qobuz')
            try:
                self.login(settings['username_or_UserID'], settings['password_or_Token'])
            except:
                    set_temporary_setting(self.session_storage_location, 'qobuz', 'hashes', None, {})
                    raise
            set_temporary_setting(self.session_storage_location, 'qobuz', 'hashes', None, hashes)
        else:
            print('Connecting to Qobuz...')
            try:
                self.session.check_subscription(settings['username_or_UserID'], settings['password_or_Token'])
            except:
                set_temporary_setting(self.session_storage_location, 'qobuz', 'hashes', None, {})
                raise

    def login(self,username_or_UserID, password_or_Token):
        token = self.session.login(username_or_UserID, password_or_Token)
    ...
    def get_album_info(self, album_id):
        ...
        return AlbumInfo(
            ...
            cover_url = album_data['image']['large'].split('_')[0] + '_org.jpg',
            all_track_cover_jpg_url = album_data['image']['large'].split('_')[0] + '_org.jpg',
            upc = album_data.get('upc'),
